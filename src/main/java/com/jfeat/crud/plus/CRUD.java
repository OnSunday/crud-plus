...
public abstract class CRUD<T> {
    ...
    /**
     * 从类型 T 中 忽略字段 fields，生成新的不包括 fields 字段的 JSONObject
     *
     * @param t
     * @param fields
     * @param <T>
     ...
     */
    @Deprecated
    public static <T> JSONObject ignoreAs(T t, String[] fields) {
    ...
    /**
     * 忽略某字段， 通常用于忽略 status 状态字段
     * @param t
     * @param fields
     * @param <T>
     * @return
     */
    public static <T> T ignore(T t, String[] fields) {
        JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(t, SerializerFeature.WriteDateUseDateFormat));
    ...
    public static void attachFrom(JSONObject target, JSONObject from, String[] selection, String prefix) {
        if (selection != null) {
    ...
    public static void attachFrom(JSONObject target, JSONObject from, String[] selection, String[] selectionMapping) {
        if (selection != null) {
    ...
    public static <T> T copyFrom(T t, T from, String[] ignoreFields, boolean override) {
        JSONObject ignored = ignoreAs(from, ignoreFields);
